...
public class GridGraph : MonoBehaviour {
    ...
    //When a building is placed, all nodes in that location become unwalkable,
    //as well as an area around it. This variable defines how much that area
    //would be.
    public float boundsExpansionAmount = 8;
    ...
    public void saveGrid()
    {
        ...
        //lines.Add("boundsExpansionAmount = \"" + boundsExpansionAmount + "\"");
    ...
    public void loadGrid(string filePath)
    {
        ...
        //heightMask = (LayerMask)int.Parse(gridData.Attributes[9].Value);
        numOfNeighbours = int.Parse(gridData.Attributes[10].Value);
        //boundsExpansionAmount = float.Parse(gridData.Attributes[11].Value);
    ...
    public void updateGrid(Bounds bounds, bool walkable)
    {
        bounds.Expand(boundsExpansionAmount);
        foreach (GridNode node in nodes)
        {
            if (bounds.Contains(node.position))
            {
                ...
                {
                    int index = ((i < 2) || (i>3 && i<6)) ? i + 2 : i - 2;
                    GridNode neighbour = nodes[node.getIndex() + neighbourOffsets[i]];
